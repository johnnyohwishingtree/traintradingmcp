...
const App = () => {
  ...
  const [editingTrendline, setEditingTrendline] = useState<{index: number; position: [number, number]} | null>(null);
  ...
  const handleAddTextButtonClick = useCallback((e: React.MouseEvent, componentType: string, componentIndex: number) => {
    ...
    if (componentType === 'trendline') {
      ...
      if (trendline) {
        // Set default text on the trendline
        ...
        const position: [number, number] = [
          (trendline.start[0] + trendline.end[0]) / 2,
          (trendline.start[1] + trendline.end[1]) / 2
        ];

        // Open inline editor immediately
        setEditingTrendline({ index: componentIndex, position });
      }
    }
  }, [trendLines]);

  // Handle inline text editing - save changes
  const handleInlineTextSave = useCallback((newText: string) => {
    if (editingTrendline && newText.trim()) {
      const updatedTrendLines = [...trendLines];
      if (updatedTrendLines[editingTrendline.index]) {
        updatedTrendLines[editingTrendline.index] = {
          ...updatedTrendLines[editingTrendline.index],
          text: newText.trim()
        };
        setTrendLines(updatedTrendLines);
  ...
  }, [editingTrendline, trendLines, trendChannels, fibonacciRetracements, trianglePatterns, labels, horizontalLines, horizontalRays, verticalLines, saveToHistory]);
  ...
  React.useEffect(() => {
    if (editingTrendline && editingInputRef.current) {
      editingInputRef.current.focus();
      editingInputRef.current.select(); // Select all text for easy replacement
    }
  }, [editingTrendline]);
  ...
  return (
    <div className="app-container">
      ...
      {editingTrendline && trendLines[editingTrendline.index] && (
        <div
          style={{
            position: 'fixed',
            top: '50%',
        ...
        >
          <div style={{ marginBottom: '8px', fontSize: '13px', fontWeight: 600, color: '#333' }}>
            Edit TrendLine Text
          </div>
          <input
            ref={editingInputRef}
            type="text"
            defaultValue={trendLines[editingTrendline.index].text || ''}
            style={{
              width: '100%',
              padding: '8px',
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                handleInlineTextSave(e.currentTarget.value);
