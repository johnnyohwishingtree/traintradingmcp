...
export class EachFibRetracement extends React.Component<EachFibRetracementProps, EachFibRetracementState> {
    ...
    private readonly checkIfHovered = (moreProps: any) => {
        const { x1, x2, y1, y2, index } = this.props;
        ...
        for (const line of lines) {
            ...
            const x2Screen = xScale(line.x2);
            const yScreen = yScale(line.y);

            const [mouseX, mouseY] = mouseXY;

            // Check if mouse is within tolerance of this fibonacci line
            const minX = Math.min(x1Screen, x2Screen) - tolerance;
            const maxX = Math.max(x1Screen, x2Screen) + tolerance;
            const minY = yScreen - tolerance;
            const maxY = yScreen + tolerance;
